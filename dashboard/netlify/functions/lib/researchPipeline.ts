...
// one conversation carried through both: a research call with the
// web_search server tool (Anthropic runs the search loop itself — no
// client-side tool_use round trips needed) produces prose findings; a
// structuring call over that conversation with no tools and a json_schema
// output format turns it into the shape the dashboard needs.
//
// An independent fact-check phase (web_fetch re-retrieving every cited
// URL) was tried and pulled back out: web_fetch consistently failed with
// url_not_in_prior_context across every domain tried, in every design
// attempted (reusing the researcher's citations, re-searching for them
// fresh) — see git history on this file. It burned real API spend without
// ever verifying a single source, so it's not worth re-attempting without
// new information about why that error was universal.
...
const NEWSROOM_STANDARDS = `Nutgraph runs a small newsroom, not a single byline — a researcher and a writer each take a pass on every assignment, both held to the same seasoned New York Times investigative standard: precise, skeptical of unverified claims, attributed rather than asserted, and unwilling to editorialize past what the evidence actually shows. This applies regardless of how casual, partisan, or unreliable the sources being cited are; the newsroom's own voice never borrows their register.
...
3. NEVER FABRICATE A SOURCE. Every "source_url" must be a real, working URL actually found via web search in this conversation — never invented, never guessed, never a plausible-looking placeholder. If there isn't enough genuine evidence to support a claim, say so rather than padding the list.
