...
// Background Function — same shape as research-topic-background.ts, but
// scoped to "what's new" on an already-published signal instead of a fresh
// topic. Writes a draft with published_signal_id set so the dashboard knows
// to open the existing signal (and append, not replace) rather than start a
// new one.
